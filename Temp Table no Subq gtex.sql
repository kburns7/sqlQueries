...
GROUP BY
	claim_tags;
	select 'rtgnGTdmr' as exclusion, sum(amt), sum(s_ic),sum(s_awp),sum(s_df), sum(amt * discount)/sum(amt)as Discount, avg(gtdiscount), sum(amt*avg_ic)/sum(amt) as avg_ic,sum(amt*avg_awp)/sum(amt) as avg_awp,sum(amt*avg_df)/sum(amt) as avg_df 
FROM
	GTcontract1
WHERE
	...
	select 'rtgnGTcob' as exclusion, sum(amt), sum(s_ic),sum(s_awp),sum(s_df), sum(amt * discount)/sum(amt)as Discount, avg(gtdiscount), sum(amt*avg_ic)/sum(amt) as avg_ic,sum(amt*avg_awp)/sum(amt) as avg_awp,sum(amt*avg_df)/sum(amt) as avg_df 
FROM
	GTcontract1
WHERE
	claim_tags LIKE'%,rtgn,%' 
